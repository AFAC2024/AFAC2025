import os
os.environ['CUDA_VISIBLE_DEVICES'] = '0,1,2,3'
import swift
from swift.llm import InferEngine, InferRequest, PtEngine, RequestConfig, load_dataset,AdapterRequest,BaseArguments,get_template
from swift.plugin import InferStats
from swift.llm import VllmEngine
import sys
import pandas as pd
import glob, json, re
import numpy as np
from tqdm import tqdm
from random import shuffle
import random
random.seed(42)

n_gpus = int(sys.argv[1])

col_dic = {
    "基础产品销售信息":"该保险产品的基础配置信息，包括产品名、附加的条款信息、销售限制等",
    "投保条款":"投保过程中的缴费约定、投被保人条件限制等",
    "保障责任":"约定该产品的保险责任细节，如保障范围、保险金额、增值服务等",
    "保障相关时间":"约定该产品的各类时间信息，包括但不限于犹豫期、等待期、宽限期等",
    "与保障相关的时间":"约定该产品的各类时间信息，包括但不限于犹豫期、等待期、宽限期等",
    "赔付 & 领取规则":"约定该产品的保险责任的赔付、给付、领取及免赔细节，如赔付年龄/比例/次数等",
    "责任免除":"约定该产品不承担保险责任的情形",
    "续保条款":"约定续保相关信息，包括但不限于续保条件、保证续保等",
    "退保条款":"约定退保相关信息，包括但不限于退保条件、退保手续费等",
    "出险条款":"约定出险相关信息，包括但不限于出险地点、出险方式等",
    "附加条款":"约定该产品的附加条款，如特别约定等",
    "术语解释":"约定该产品的术语解释，如名词定义等"
}

def infer_batch(qs):
    infer_requests = [InferRequest(messages=[{'role': 'user', 'content':q}]) for q in qs]
    request_config = RequestConfig(max_tokens=100,temperature=0)
    resp_list = engine.infer(infer_requests,request_config)
    res = [x.choices[0].message.content for x in resp_list]
    res = [get_res(x) for x in res]
    return res

def get_res(text):
    if "</think>" in text:
        text = text[text.find("</think>")+8:].strip().replace("<|endoftext|>","").strip()
    return text


def exteract_module_text_prompt(module_name, lines):
    prompt = """
    你是一个专业的保险行业的信息处理专家,请你基于产品背景材料回答用户问题。如果没有答案，请返回"没有相关信息",请直接返回答案，无需额外说明。
    用户问题：{}
    产品背景材料：{}
    """
    p = prompt.format("该产品的{}({})是什么？".format(module_name,col_dic[module_name]),lines).strip()
    return p[:max_background_length]

def analysis_conflict_prompt(module_name, content):
    # prompt = """
    # 你是一个专业的保险行业的信息处理专家,请你判断产品材料信息中关于{}的描述信息是否有明显冲突，请直接返回是或者否，无需额外说明。
    # 产品材料信息：
    # {}
    # """
    prompt = """
    你是一个专业的保险行业的信息处理专家,请你判断产品材料信息中关于{}的描述信息是否有冲突矛盾之处，请直接返回是或者否，无需额外说明。
    产品材料信息：
    {}
    """
    
    content = get_res(content)
    p = prompt.format("{}({})".format(module_name,col_dic[module_name]),content).strip()
    return p


if __name__ == '__main__':
    model = "eval_model/qwen2.5-14b/checkpoint-885-merged"
    engine = VllmEngine(model,gpu_memory_utilization=0.95,tensor_parallel_size=n_gpus,seed=42,max_model_len = 580000)
    dic = ["责任免除","基础产品销售信息","保障责任"]
    info = []
    for row in tqdm(pd.read_json("测试 B 集/data.jsonl", lines=True).iloc[:].iterrows(),total=240):
        module_name = row[1].rule.replace("该产品的", "").replace("在各材料中的定义没有冲突", "")
        module_content = {}

        for path in glob.glob(f"测试 B 集/materials/{row[1].material_id}/*/*"):
            lines = open(path).readlines()
            lines = "".join(lines)
            colunm = path.strip().split("/")[-2].strip()
            if colunm in module_content:
                module_content[colunm] = module_content[colunm]+"\n\n"+lines
            else:
                module_content[colunm] = lines

        res = []
        docs = [x+"材料\n"+y[:450000] for x,y in module_content.items()]
        for i in range(5):
            content = "\n\n".join(docs)
            q = analysis_conflict_prompt(module_name,content)
            r = infer_batch([q])[0]
            res.append(r)
            shuffle(docs)
        
        tmp_info = {"material_id":row[1].material_id,"module_name":module_name,"result":res}
        info.append(tmp_info)

    json.dump(info,open("files/result_info_v3_b.json","w"),ensure_ascii=False,indent=4)

    info = json.load(open("files/result_info_v3_b.json","r"))
    
    n=0
    tmp = []
    result = []
    data = [json.loads(x.strip()) for x in open("测试 B 集/data.jsonl")]
    for i in range(len(data)):
        item = data[i]
        # del item["rule"]
        r = [1 if "是" in x else 0 for x in info[i]["result"]]
        if sum(r)>3:
            item["result"] = False
        else:
            n+=1
            item["result"] = True

        result.append(item)
    
    with open("files/submit_v3_b.jsonl","w") as f:
        for x in result:
            f.write("{}\n".format(json.dumps(x,ensure_ascii=False)))
    print(n)
