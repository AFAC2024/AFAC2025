...
class SimpleTMModel(nn.Module):
    def __init__(self, configs):
        super(SimpleTMModel, self).__init__()
    ...
    def forward(self, x_enc, y, x_mark_enc=None, x_dec=None, x_mark_dec=None):
        dec_out, attns = self.forecast(x_enc, None, None, None)
        loss=None
        if y is not None:
            b,s,f = dec_out.shape
            loss = F.l1_loss(dec_out[:,:,-2:].reshape(b, s * 2),y[:,:,-2:].reshape(b, s * 2))
        return dec_out, loss