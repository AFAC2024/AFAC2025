...
class NaiveRewardManager:
    ...
    def __init__(self, tokenizer, num_examine, train_batch_size, num_generation, compute_score=None) -> None:
    ...
    def check_correctness_and_length(self, data):
        ...
        for problem_id in unique_problem_ids:
            ...
            for idx, completion in zip(indices, completions_given_prompt):
                ...
                # 新增：打印错误题目 id、回答和标准答案
                # if not correct_or_not:
                #     print(f"[错误题目ID] {completion.non_tensor_batch['index']}")
                #     print(f"[模型回答] {response_str[-8:]}")
            ...
            optimal_length = sum(response_lengths_given_prompt)/len(response_lengths_given_prompt)
            
            # 计算标准差
            import math
            mean_length = optimal_length
            variance = sum((length - mean_length) ** 2 for length in response_lengths_given_prompt) / len(response_lengths_given_prompt)
            ...
            # 新增：如果全部回答都错误，打印题目ID和所有模型回答
            if correct_count == 0:
                print(f"[全部错误题目ID] {problem_id}")
                print(f"[标准答案] {ground_truth}")
            # Store optimal length and std for all instances of this problem
            for idx in indices:
                optimal_length_map[idx] = optimal_length
                length_std_map[idx] = length_std
        ...
        length_std_set = [length_std_map[i] for i in range(len(data))]
        
        assert len(correctness_set) == len(length_set) == len(optimal_length_set) == len(length_std_set) == len(data), f"Implementation Error: the length of correctness_set={len(correctness_set)}, length_set={len(length_set)}, optimal_length_set={len(optimal_length_set)}, length_std_set={len(length_std_set)}, data={len(data)}, they should all be the same length"
        
        return correctness_set, length_set, optimal_length_set, length_std_set
    ...
    def __call__(self, data: DataProto):
        ...
        correctness_set, length_set, optimal_length_set, length_std_set = self.check_correctness_and_length(data)
        ...
        #     else:
        #         correctness = 0.0
        #     if completion_length <= optimal_length:
        #         length_gap = 0.0
        #     else:
        #         length_gap = abs(completion_length - optimal_length) * beta

        #     return correctness - length_gap
        def sb_compute_score(optimal_length, completion_length, length_std, correct_or_not):
            """return the reward score based on sb func; optimal length is the shortest length among all correct completions"""
            alpha = 0.2  # 长度惩罚权重，范围[0,1]
            
            # 正确性部分
            correctness = 1.0 if correct_or_not else 0.0
            
            # 使用标准差进行归一化
            length_penalty = (completion_length - optimal_length) / length_std
            
            # 应用公式：正确性 * (1 - α * 长度惩罚)
            reward = correctness * (1 - alpha * length_penalty)
        ...
        for i in range(len(data)):
            ...
            score = sb_compute_score(optimal_length, completion_length, length_std, correct_or_not)
...
def verify_correctness(solution_str, ground_truth) -> float:
    ...
    try:
        ...
        if string_in_last_boxed is not None:
            ...
            # Check if it's a multiple choice question (contains only letters)
            if is_multiple_choice(answer, ground_truth):
                if is_choice_equiv(answer, ground_truth):
                    retval = True
            else:
                # Original math equivalence check
...
def is_multiple_choice(answer, ground_truth):
    """
    判断是否为选择题：答案和ground_truth都只包含英文字母和常见分隔符
    """
    import re
    # 允许字母、空格、逗号、分号等常见分隔符
    pattern = r'^[A-Za-z\s,;]+$'
    return bool(re.match(pattern, answer.strip())) and bool(re.match(pattern, ground_truth.strip()))


def is_choice_equiv(answer, ground_truth):
    """
    比较选择题答案是否等价（不考虑顺序，只要字母组合一致）
    """
    ...
    try:
        answer_letters = normalize_choices(answer)
        ground_truth_letters = normalize_choices(ground_truth)
        
        # 比较字母集合是否相同
        return answer_letters == ground_truth_letters
    except Exception:
        return False
