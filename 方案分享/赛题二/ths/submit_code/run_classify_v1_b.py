...
if __name__ == '__main__':
    ...
    engine = VllmEngine(model,gpu_memory_utilization=0.9,tensor_parallel_size=n_gpus,seed=42,max_model_len=580000)
    
    data = json.load(open("files/rag_answer_b.json","r"))
    result = []
    for x in tqdm(data):
        new_compare = []
    ...
    json.dump(result,open("files/result_info_v1_b.json","w"),ensure_ascii=False,indent=4)
        
    
    data = [json.loads(x.strip()) for x in open("测试 B 集/data.jsonl")]
    info = json.load(open("files/result_info_v1_b.json","r"))
    ...
    for i in range(len(data)):
        item = data[i]
        rule = item["rule"].replace("该产品的", "").replace("在各材料中的定义没有冲突", "")
    ...
    with open("files/submit_v1_b.jsonl","w") as f:
        for x in result:
