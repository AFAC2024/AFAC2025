...
#         # 调用模型获取响应
#         model_response = resres(text_input,0.25)
#         model_response = model_response.replace("\n","")
#         # 提取模型回答的答案
#         model_answer = extract_boxed_answer(model_response)
        
#         # 判断是否正确
#         if model_answer == item['output']:
#             # 如果正确，更新output为完整模型响应
#             item['output'] = model_response
#         else:
#             # 如果错误，在原始output后添加"(回答错误)"
#             item['output'] = f"{item['output']}（回答错误）"
...
def process_json_file(input_file, output_file):
    ...
    for item in data:
        ...
        model_response2 = resres(text_input2, 0.8)
        ...
        # 第三次尝试：提供正确答案并要求解释
        text_input3 = f'''{item['instruction']}\n{item['input']}\n\n现在我告诉你，答案是 {original_output}，请你帮我解释为什么答案是这个，并给出你的最终答案，答案仍然要用 \boxed{{}} 的形式输出。'''
...
# 使用示例
process_json_file('fineval-train-ds.json', 'fineval-train-ds-cot.json')
