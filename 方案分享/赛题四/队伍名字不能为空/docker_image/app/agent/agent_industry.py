...
from prompts import stock_prompts, industry_prompts
...
async def get_data_date(question: str):
    ...
    if cache is not None:
      ...
      msgs = stock_prompts['tpl_get_data_date'].invoke({"roles": "股票分析师", "target": "行业", "msg": question}).to_messages()
...
async def get_industry_info(question: str):
    cache_key = 'get_industry_info,{0}'.format(question)
    cache = getCache(cache_key)
    if cache is not None:
      return cache
    msgs = industry_prompts['tpl_get_industry_info'].invoke({"roles": "股票分析师", "target": "行业", "msg": question}).to_messages()
    res = await llm_invoke(msgs, call_type='agent')
    setCache(cache_key, res)
    return res

async def get_outline(desc: str):
    cache_key = 'get_outline_industry,{0}'.format(desc)
    cache = getCache(cache_key)
    if cache is not None:
      cache = json.loads(cache)
      return cache
    msgs = industry_prompts['tpl_get_outline'].invoke({
       "roles": "资深财经主编",
       "target": "行业",
...
async def get_info_data(desc, outlines):
    ...
    msgs = industry_prompts['tpl_get_info_data'].invoke({"roles": "资深财经编辑、专业数据分析师", "target": "行业", "desc": desc}).to_messages()

    cache_key = 'get_info_data,{0}'.format(str(msgs))
    cache = getCache(cache_key)
    if cache is not None:
       cache = json.loads(cache)
       return cache
...
async def get_section_content(section, standinfo, data_date, doc_idx=1):
  content = ''
  msg = industry_prompts['tpl_topic_info'].format(target=standinfo['name'] + '行业'
                                               , title=section['title']
                                               , desc=section['desc']
                                               , requirements=str(section['requirements']))
  summary_list = await agent_data.get_section_docs(msg, target='行业', start_idx=doc_idx)
  ...
  msgs = industry_prompts['tpl_general_section_content'].invoke({
...
async def get_key_words(outlines):
  ...
  outlines_txt = '\n\n'.join(outlines_txt)
  msgs = industry_prompts['tpl_get_key_words'].invoke({"roles": "资深财经编辑、专业数据分析师"
                                                    , "target": "行业"
                                                    , "outlines": outlines_txt}).to_messages()

  cache_key = 'get_key_words_industry,{0}'.format(str(msgs))
  cache = getCache(cache_key)
  if cache is not None:
...
async def beautyfy_content(comp_info, content_list):
  ...
  msgs = industry_prompts['tpl_beauty_section_content'].invoke({
...
async def agent_executor(question: str):
  ...
  desc = await get_industry_info(question)
  print(desc)

  if '请提供' in desc and '生成研报' in desc and '行业' in desc and '名称' in desc:
    logging.error(desc)
    raise ValueError("抱歉，未能识别到您提供的行业信息，请试试提供完整的行业名称。")
  ...
  data_len = await agent_data.search_docs(key_words, stand_data['name'] + '研报\n\n' + content_info, data_date)
  print(data_len)
  last_success_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
  ...
  tasks = []
  for section in res_list:
    topic = stock_prompts['tpl_topic_info'].format(target=stand_data['name']
                                                   , title=section['title']
                                                   , desc=section['desc']
                                                   , requirements=str(section['requirements']))
    tasks.append((agent_chart.get_paragraph_chart_worker, (topic, section, all_sources, '行业'), {}))
  ...
  # file_path = '/Users/nambo/Documents/project/tianchi/20250714AFAC2025/mcps/common/_cache/report_stock_img_2025072315:53:49__328ec8607c2a4dfebb37d504ea5cff91'
  # with open(file_path, 'r', encoding='utf-8') as f:
  #       file_content = f.read()
  # res_list = json.loads(file_content)

  industry_name = stand_data['name']
  if '行业' not in industry_name:
     industry_name += '行业'

  report_name = '{0}\n证券研究报告'.format(industry_name)
  file_name = SAVE_DIR + '/Industry_Research_Report.docx'
  
  return create_document.generate_report(res_list, file_name, report_name, data_date, target='行业', target_name=industry_name )
  return res_list
...
async def main():
  res = await agent_executor("我需要生成“中国智能服务机器人产业”的研报")
  print(res)
