...
class GeomAttentionLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask=None, tau=None, delta=None):
        ...
        #queries:[32,7,m+1,32],这里config中m=3
        #keys:[32,7,m+1,32],这里config中m=3
        #values:[32,7,m+1,32],这里config中m=3
        #线性变换获取QKV矩阵
...
class GeomAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask=None):
        ...
        # queries: [B, L, H, E] = [32,32,4,7]
        # keys: [B, S, H, E]
        #相当于是一个标准的多头注意力，以SWT得到的特征数作为头的数量
        dot_product = torch.einsum("blhe,bshe->bhls", queries, keys)
        #得到的dot_product矩阵维度是[32, 4, 32, 32]
