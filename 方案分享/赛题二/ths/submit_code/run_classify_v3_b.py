...
if __name__ == '__main__':
    model = "output/qwen2.5-14b/checkpoint-885-merged"
    engine = VllmEngine(model,gpu_memory_utilization=0.95,tensor_parallel_size=4,seed=42)
    ...
    for row in tqdm(pd.read_json("测试 B 集/data.jsonl", lines=True).iloc[:].iterrows(),total=240):
        ...
        tmp_info = {"material_id":row[1].material_id,"module_name":module_name,"result":res,"context_length":len(q)}
